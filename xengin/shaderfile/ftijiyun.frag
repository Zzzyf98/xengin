...
#define top 20      // 云层顶部
#define width 200     // 云层 xz 坐标范围 [-width, width]
...
float getDensity(sampler2D noisetex, vec3 pos) {
    ...
    float w1 = abs(abs(pos.x)-width);
    float w2 = abs(abs(pos.z)-width);

    float weight2 = 1.0f;
    if(w1 < 50) weight2 = w1/50;
    if(w2 < 50) weight2 *= w2/50;



    // 采样噪声图
    vec2 coord = pos.xz * 0.0025;
    coord.x += float(frameCounter)*0.00001;
    ...
    if(w1 < 50) noise = 100.0f;
    if(w2 < 50) noise = 100.0f;
